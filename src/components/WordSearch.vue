<template>
  <div>
    ...
    <div class="word-list-container">
      <h2>Word List</h2>
        <transition-group name="slideRight" class="word-list" tag="ul">
    ...
    <div class="results-container">
      ...
      <ul v-if="results && results.length > 0" class="results">
        <transition-group name="bounceDown" tag="div" appear>
      ...
      <div v-else-i="results && results.length === 0" class="resultsnotfound">
        <h2>No results found. Try a new search.</h2>
      </div>
    </div>
  </div>
</template>
  ...
  methods: {
    addWord: function (word) {
      if (this.wordList.indexOf(word) === -1) {
        ...
        this.messages.push({
          type:'success',
          text: `${word} has been added to your Wordlist`
        });
       } else {
        console.log('Word is already on wordlist.');
        this.messages.push({
          type:'info',
          text: `${word} is already in your Wordlist`
        });
        }
    },
    removeWord: function (word) {
      ...
      this.messages.push({
        type:'success',
        text:`${word} has been removed from your Wordlist`
      });
    },
    findWords: function() {
      this.results = null;
